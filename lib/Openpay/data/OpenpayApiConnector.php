...
class OpenpayApiConnector {
	...
	private function _request($method, $url, $params) {
        try{
            $classDoesExist = class_exists('Openpay');
        }catch (Exception $e){
	...
	private function _curlRequest($method, $absUrl, $headers, $params, $auth=null) {
        ...
        // Solve issue with class_exists not looking for file exists in the first place
		try{
            $classDoesExist = class_exists('OpenpayApiTestResponses');
        }catch (Exception $e){
            $classDoesExist = false;
        }

		if ($classDoesExist && OpenpayApiTestResponses::SIMULATE == true) {
